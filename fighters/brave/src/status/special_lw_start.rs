...
unsafe extern "C" fn brave_special_lw_start_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let mask = VarModule::get_int(fighter.module_accessor, vars::brave::instance::int::USED_SPELL_MASK) | (1 << spell_kind);
    // println!("New Mask: {:#b}", mask);
    VarModule::set_int(fighter.module_accessor, vars::brave::instance::int::USED_SPELL_MASK, mask);
    ...
    VarModule::set_int(fighter.module_accessor, vars::brave::instance::int::SPELL_SLOT_1 + index, -1);
    for x in 0..21 {
        if mask & (1 << x) == 0 {
            break;
        }
