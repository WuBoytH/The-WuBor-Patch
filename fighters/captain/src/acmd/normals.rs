...
unsafe extern "C" fn game_attackdash(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
        let boost_power = VarModule::get_float(agent.module_accessor, vars::captain::instance::float::BOOST_POWER);
        let ratio = boost_power / vars::captain::BOOST_POWER_MAX;
        let mul = 1.0 + (0.5 * ratio);
        sv_kinetic_energy!(
            set_speed_mul,
            agent,
