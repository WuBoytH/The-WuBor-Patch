...
unsafe extern "C" fn captain_special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let manual_input = VarModule::is_flag(fighter.module_accessor, vars::captain::status::flag::SPECIAL_S_ENABLE_MANUAL_ATTACK)
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_S, captain_special_s_main);
}