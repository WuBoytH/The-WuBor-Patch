...
unsafe extern "C" fn taunt_uniq_handler(fighter: &mut L2CFighterCommon) {
    if VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUTTON_CHECK) {
        let action_button = VarModule::get_int(fighter.module_accessor, vars::appeal::int::ACTION_BUTTON);
        if ControlModule::check_button_on_trriger(fighter.module_accessor, action_button) {
            let flag = if VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUTTON_ENABLE_SUCCESS) {
                vars::appeal::flag::ACTION_BUFFER_SUCCESS
            }
            else {
                vars::appeal::flag::ACTION_BUFFER_LOCKED
            };
            VarModule::on_flag(fighter.module_accessor, flag);
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUTTON_CHECK);
        }
    }
    if VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::HOLD)
    || VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::LOOP) {
        // New logic for taunt holds/loops/actions.
        let loop_mot = VarModule::get_int64(fighter.module_accessor, vars::appeal::int64::LOOP_MOT);
        ...
        let is_loop = VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::LOOP);
        taunt_holds(fighter, is_loop);
    }
    if VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ENABLE_ACTION)
    || VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ENABLE_ACTION_IMM) {
        let action_button = VarModule::get_int(fighter.module_accessor, vars::appeal::int::ACTION_BUTTON);
        if !VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUFFER_LOCKED)
        && (
            VarModule::is_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUFFER_SUCCESS)
            || ControlModule::check_button_on_trriger(fighter.module_accessor, action_button)
        ) {
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::HOLD);
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::LOOP);
            let action_mot = VarModule::get_int64(fighter.module_accessor, vars::appeal::int64::ACTION_MOT);
            let action_frame = VarModule::get_int(fighter.module_accessor, vars::appeal::int::ACTION_FRAME);
            MotionModule::change_motion(
                fighter.module_accessor,
            ...
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUFFER_SUCCESS);
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUFFER_LOCKED);
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ACTION_BUTTON_ENABLE_SUCCESS);
            VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ENABLE_ACTION);
        }
        VarModule::off_flag(fighter.module_accessor, vars::appeal::flag::ENABLE_ACTION_IMM);
    }
}
...
unsafe extern "C" fn taunt_holds(fighter: &mut L2CFighterCommon, is_loop: bool) {
    let button = VarModule::get_int(fighter.module_accessor, vars::appeal::int::HOLD_BUTTON);
    ...
    && (
        ...
        let restart_frame = VarModule::get_int(fighter.module_accessor, vars::appeal::int::RESTART_FRAME) as f32;
