...
unsafe extern "C" fn status_pre_damageflyroll(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    VarModule::set_flag(fighter.module_accessor, vars::damage_fly_roll::flag::DISABLE_PASSIVE, disable_passive);
    0.into()
}
...
unsafe extern "C" fn status_damageflyroll_common(fighter: &mut L2CFighterCommon) {
    ...
    for x in passives.iter() {
        let able = if VarModule::is_flag(fighter.module_accessor, vars::damage_fly_roll::flag::DISABLE_PASSIVE) {
