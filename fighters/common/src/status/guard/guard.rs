...
unsafe extern "C" fn sub_guard_cont_pre(fighter: &mut L2CFighterCommon) {
    ...
    if fighter.global_table[STATUS_KIND_INTERRUPT].get_i32() == *FIGHTER_STATUS_KIND_GUARD_ON {
        ...
        if fighter.global_table[PREV_STATUS_KIND].get_i32() == *FIGHTER_STATUS_KIND_RUN {
...
unsafe extern "C" fn sub_guard_cont(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && ItemModule::is_have_item(fighter.module_accessor, 0) && {
        ...
        && fighter.global_table[CMD_CAT3].get_i32() & (*FIGHTER_PAD_CMD_CAT3_ITEM_LIGHT_THROW_HI | *FIGHTER_PAD_CMD_CAT3_ITEM_LIGHT_THROW_HI4) != 0) {
            if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
                set_cat1_backup(fighter, 0, true);
    ...
    if can_act {
        if ControlModule::check_button_on_trriger(fighter.module_accessor, *CONTROL_PAD_BUTTON_CSTICK_ON) {
        ...
        || fighter.sub_check_button_frick().get_bool() {
            set_cat1_backup(fighter, *FIGHTER_PAD_CMD_CAT1_FLAG_JUMP | *FIGHTER_PAD_CMD_CAT1_FLAG_JUMP_BUTTON, false);
            fighter.change_status(FIGHTER_STATUS_KIND_GUARD_OFF.into(), false.into());
            return true.into();
        }
    }
...
unsafe extern "C" fn set_cat1_backup(fighter: &mut L2CFighterCommon, mut reserve_cat: i32, clear_catch: bool) {
    VarModule::on_flag(fighter.module_accessor, vars::guard::flag::ADD_BUFFER);
    if clear_catch {
        ControlModule::clear_command_one(fighter.module_accessor, 0, 0x1d);
        if reserve_cat & *FIGHTER_PAD_CMD_CAT1_FLAG_CATCH != 0 {
            reserve_cat -= *FIGHTER_PAD_CMD_CAT1_FLAG_CATCH;
        }
    }
    VarModule::set_int(fighter.module_accessor, vars::guard::int::GUARD_OFF_RESERVE_CAT1, reserve_cat);
}
...
unsafe extern "C" fn check_guard_attack_special_hi(fighter: &mut L2CFighterCommon, guard_hold: L2CValue) -> L2CValue {
    ...
    if !guard_hold.get_bool() {
        ...
        && fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND
        && !ItemModule::is_have_item(fighter.module_accessor, 0) {
            set_cat1_backup(fighter, *FIGHTER_PAD_CMD_CAT1_FLAG_ATTACK_HI4, true);
            fighter.change_status(FIGHTER_STATUS_KIND_GUARD_OFF.into(), false.into());
            return true.into();
        }
    }
    ...
    || stick_y > special_stick_y && flick_y < jump_flick_y {
        ...
        && cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI != 0 {
            ...
            if cont {
                ...
                set_cat1_backup(fighter, *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI, false);
                fighter.change_status(FIGHTER_STATUS_KIND_GUARD_OFF.into(), false.into());
                return true.into();
            }
        }
