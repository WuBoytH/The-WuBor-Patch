...
unsafe extern "C" fn status_dashcommon(fighter: &mut L2CFighterCommon) {
    ...
    VarModule::off_flag(fighter.module_accessor, vars::dash::flag::DISABLE_RUN);
...
unsafe extern "C" fn sub_dash_uniq_process_main_internal(fighter: &mut L2CFighterCommon, param_1: L2CValue) {
    if !WorkModule::is_enable_transition_term_forbid(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ATTACK_DASH) {
        ...
        if stick_x * lr * is_backdash < walk_threshold {
            ...
            VarModule::on_flag(fighter.module_accessor, vars::dash::flag::DISABLE_RUN);
        }
    }
    ...
    if WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_STATUS_DASH_WORK_INT_TURN_DASH_FRAME, 0) {
    ...
    if WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_STATUS_DASH_WORK_INT_RETRY_TURN_DASH_FRAME, 0) {
    ...
    if WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_STATUS_DASH_WORK_INT_ENABLE_ATTACK_FRAME, 0) {
    ...
    if WorkModule::count_down_int(fighter.module_accessor, *FIGHTER_STATUS_DASH_WORK_INT_INVALID_ATTACK_ESCAPE_FRAME, 0) {
        let transitions = [
            *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ATTACK_HI4_START,
            *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_ATTACK_S4_START,
            *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_SPECIAL_S,
...
unsafe extern "C" fn check_dash_cancel_early_return(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::fighter::status::flag::IS_DASH_CANCEL) {
        if WorkModule::is_enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_DASH_TO_RUN) {
            VarModule::off_flag(fighter.module_accessor, vars::fighter::status::flag::IS_DASH_CANCEL);
