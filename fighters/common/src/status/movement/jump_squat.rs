...
unsafe extern "C" fn sub_jump_squat_uniq_process_init_param(fighter: &mut L2CFighterCommon, param_1: L2CValue) {
    VarModule::on_flag(fighter.module_accessor, vars::fighter::instance::flag::JUMP_FROM_SQUAT);
...
unsafe extern "C" fn jump_squat_check_special_jump(fighter: &mut L2CFighterCommon) {
    ...
    && fighter.global_table[FLICK_Y_DIR].get_f32() < 0.0) {
        VarModule::on_flag(fighter.module_accessor, vars::fighter::instance::flag::SUPER_JUMP);
    }
    else {
