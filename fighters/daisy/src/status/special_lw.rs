...
unsafe extern "C" fn daisy_speciallw_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        ...
        WorkModule::set_int64(fighter.module_accessor, *ITEM_KIND_NONE as u64, *FIGHTER_PEACH_STATUS_SPECIAL_LW_WORK_INT_UNIQ_ITEM_KIND);
    }
    0.into()
}

pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_LW, daisy_speciallw_pre);
    agent.status(Init, *FIGHTER_STATUS_KIND_SPECIAL_LW, daisy_speciallw_init);
}
