...
unsafe extern "C" fn demon_attack_stand_2_get_mot(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    match combo {
        ...
        1 => {
            if VarModule::is_flag(fighter.module_accessor, vars::demon::status::flag::ATTACK_STAND_2_SPECIAL_FINISHER) {
...
unsafe extern "C" fn demon_attack_stand_2_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && !StatusModule::is_changing(fighter.module_accessor) {
        if ControlModule::check_button_trigger(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL) {
            VarModule::on_flag(fighter.module_accessor, vars::demon::status::flag::ATTACK_STAND_2_SPECIAL_FINISHER);
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_DEMON_STATUS_KIND_ATTACK_STAND_2, demon_attack_stand_2_main);
}