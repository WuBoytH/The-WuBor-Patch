...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, vars::dolly::status::SPECIAL_LW_BREAKING, special_lw_breaking_pre);
    agent.status(Main, vars::dolly::status::SPECIAL_LW_BREAKING, special_lw_breaking_main);
}