...
unsafe extern "C" fn dolly_special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        VarModule::on_flag(fighter.module_accessor, vars::dolly::status::flag::SPECIAL_N_CHECK_FEINT);
    }
...
unsafe extern "C" fn special_n_substatus(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if !param_1.get_bool() {
        if VarModule::is_flag(fighter.module_accessor, vars::dolly::status::flag::SPECIAL_N_CHECK_FEINT)
        && ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_GUARD) {
            VarModule::on_flag(fighter.module_accessor, vars::dolly::status::flag::SPECIAL_N_FEINT);
        }
    }
    0.into()
}
...
unsafe extern "C" fn dolly_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::dolly::status::flag::SPECIAL_N_FEINT)
    && MotionModule::motion_kind(fighter.module_accessor) == hash40("special_n") {
        MotionModule::change_motion_inherit_frame(
            fighter.module_accessor,
            Hash40::new("special_n_feint"),
            -1.0,
            1.0,
            0.0,
            false,
...
unsafe extern "C" fn dolly_special_n_end(fighter: &mut L2CFighterCommon) -> L2CValue {
