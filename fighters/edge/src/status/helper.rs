...
pub unsafe extern "C" fn edge_special_hi_cancel(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::edge::status::flag::SPECIAL_HI_CANCEL)
    && VarModule::get_int(fighter.module_accessor, vars::edge::instance::int::SPECIAL_HI_CANCEL_COUNT) < 2 {
        VarModule::on_flag(fighter.module_accessor, vars::fighter::status::flag::FORCE_ESCAPE_AIR_SLIDE_IN_STATUS);
        let cat1 = fighter.global_table[CMD_CAT1].get_i32();
        if cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI != 0 {
            VarModule::inc_int(fighter.module_accessor, vars::edge::instance::int::SPECIAL_HI_CANCEL_COUNT);
        ...
        || fighter.sub_transition_group_check_air_jump_aerial().get_bool() {
            VarModule::set_int(fighter.module_accessor, vars::edge::instance::int::SPECIAL_HI_CANCEL_COUNT, 2);
            return true.into();
        }
