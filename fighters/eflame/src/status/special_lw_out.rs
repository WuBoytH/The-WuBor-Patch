...
pub unsafe extern "C" fn element_special_lw_out_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    VarModule::set_flag(fighter.module_accessor, vars::element::status::flag::SPECIAL_LW_OUT_ATTACK, is_attack);
...
unsafe extern "C" fn element_special_lw_out_mot_helper(fighter: &mut L2CFighterCommon, first: bool) {
    let is_attack = VarModule::is_flag(fighter.module_accessor, vars::element::status::flag::SPECIAL_LW_OUT_ATTACK);
...
unsafe extern "C" fn element_special_lw_out_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_AIR {
        if VarModule::is_flag(fighter.module_accessor, vars::element::status::flag::SPECIAL_LW_OUT_ATTACK_FALL) {
            let air_speed_y_stable = WorkModule::get_param_float(fighter.module_accessor, hash40("air_speed_y_stable"), 0);
            sv_kinetic_energy!(
                set_stable_speed,
                fighter,
            ...
            VarModule::off_flag(fighter.module_accessor, vars::element::status::flag::SPECIAL_LW_OUT_ATTACK_FALL);
        }
