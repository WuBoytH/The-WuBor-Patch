...
unsafe extern "C" fn fox_special_hi_rush_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StopModule::is_stop(fighter.module_accessor) {
        fox_special_hi_rush_substatus(fighter);
    }
    fighter.global_table[SUB_STATUS2].assign(&L2CValue::Ptr(fox_special_hi_rush_substatus as *const () as _));
    fighter.sub_shift_status_main(L2CValue::Ptr(fox_special_hi_rush_main_loop as *const () as _))
}

unsafe extern "C" fn fox_special_hi_rush_substatus(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn fox_special_hi_rush_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::fox::status::flag::SPECIAL_HI_ENABLE_SNAP) {
        VarModule::off_flag(fighter.module_accessor, vars::fox::status::flag::SPECIAL_HI_ENABLE_SNAP);
        if situation == *SITUATION_KIND_AIR {
            fighter.sub_fighter_cliff_check(GROUND_CLIFF_CHECK_KIND_ALWAYS_BOTH_SIDES.into());
        }
        VarModule::on_flag(fighter.module_accessor, vars::fox::status::flag::SPECIAL_HI_ENABLED_SNAP);
    }
    ...
    || (
        ...
        if situation != *SITUATION_KIND_GROUND {
            ...
            let check_kind = if VarModule::is_flag(fighter.module_accessor, vars::fox::status::flag::SPECIAL_HI_ENABLED_SNAP) {
    ...
    if !fighter.global_table[IS_STOP].get_bool() {
        fox_special_hi_rush_handle_bound(fighter);
    }
    0.into()
}

unsafe extern "C" fn fox_special_hi_rush_handle_bound(fighter: &mut L2CFighterCommon) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_FOX_STATUS_KIND_SPECIAL_HI_RUSH, fox_special_hi_rush_main);
}