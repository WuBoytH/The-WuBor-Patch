...
pub unsafe extern "C" fn deception_feint_handler(fighter: &mut L2CFighterCommon) {
    if VarModule::is_flag(fighter.module_accessor, vars::ganon::status::flag::TELEPORT_FEINT) {
        ...
        let og = VarModule::get_vec2(fighter.module_accessor, vars::ganon::status::float::TELEPORT_START_POS);
        GroundModule::set_passable_check(fighter.module_accessor, true);
        PostureModule::set_pos_2d(fighter.module_accessor, &Vector2f {x: og.x, y: og.y});
        if VarModule::is_flag(fighter.module_accessor, vars::ganon::status::flag::TELEPORT_START_GROUND) {
        ...
        let tele_y = VarModule::get_vec2(fighter.module_accessor, vars::ganon::status::float::TELEPORT_END_POS).y;
        if VarModule::is_flag(fighter.module_accessor, vars::ganon::status::flag::TELEPORT_START_GROUND)
        && tele_y == 0.0 {
            GroundModule::set_correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
        }
    ...
    VarModule::set_int(fighter.module_accessor, vars::ganon::status::int::TELEPORT_STEP, vars::ganon::TELEPORT_STEP_END);
}
