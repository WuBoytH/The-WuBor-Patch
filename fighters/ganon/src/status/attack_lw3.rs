...
unsafe extern "C" fn ganon_attack_lw3_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.status_AttackLw3_common();
    fighter.sub_shift_status_main(L2CValue::Ptr(ganon_attack_lw3_main_loop as *const () as _))
}

unsafe extern "C" fn ganon_attack_lw3_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_ATTACK_LW3, ganon_attack_lw3_main);
    agent.status(End, *FIGHTER_STATUS_KIND_ATTACK_LW3, ganon_attack_lw3_end);
}