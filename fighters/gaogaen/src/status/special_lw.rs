...
unsafe extern "C" fn gaogaen_special_lw_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::off_flag(fighter.module_accessor, vars::gaogaen::status::flag::REVENGE_AUTO);
    let mot;
    let correct;
...
unsafe extern "C" fn gaogaen_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::gaogaen::status::flag::REVENGE_AUTO) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_LW, gaogaen_special_lw_main);
}