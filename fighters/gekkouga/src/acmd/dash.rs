...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 1.0);
    if macros::is_excute(agent) {
        macros::PLAY_STEP(agent, Hash40::new("se_gekkouga_dash_start"));
        macros::SET_PLAY_INHIVIT(agent, Hash40::new("se_gekkouga_dash_start"), 20);
    }
    wait(agent.lua_state_agent, 17.0);
    if macros::is_excute(agent) {
        macros::PLAY_STEP(agent, Hash40::new("se_gekkouga_step_right_l"));
    }
    wait(agent.lua_state_agent, 5.0);
    if macros::is_excute(agent) {
        macros::PLAY_STEP(agent, Hash40::new("se_gekkouga_step_left_l"));
    }
}
