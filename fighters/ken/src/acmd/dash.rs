...
unsafe extern "C" fn effect_dash(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::FOOT_EFFECT(agent, Hash40::new("sys_dash_smoke"), Hash40::new("top"), -1.5, 0, 0, 0, 0, 0, 0.7, 0, 0, 0, 0, 0, 0, false);
    }
...
unsafe extern "C" fn sound_dash(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 1.0);
    if macros::is_excute(agent) {
        macros::PLAY_SE(agent, Hash40::new("se_ken_dash_start"));
        macros::SET_PLAY_INHIVIT(agent, Hash40::new("se_ken_dash_start"), 10);
    }
}
