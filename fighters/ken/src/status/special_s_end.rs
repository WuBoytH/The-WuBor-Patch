...
unsafe extern "C" fn ken_special_s_end_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    || StatusModule::is_situation_changed(fighter.module_accessor) {
        ...
        if sit != *SITUATION_KIND_GROUND {
            ...
            let air_end_brake_x = if VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
                0.01
            }
            else {
            ...
            let end_air_max_speed_y = if !VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
            ...
            let end_air_accel_y = if !VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
            ...
            if VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_RYU_STATUS_KIND_SPECIAL_S_END, ken_special_s_end_main);
    agent.status(End, *FIGHTER_RYU_STATUS_KIND_SPECIAL_S_END, ken_special_s_end_end);
}