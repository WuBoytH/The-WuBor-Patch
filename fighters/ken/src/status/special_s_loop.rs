...
unsafe extern "C" fn ken_special_s_loop_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if current_sit != *SITUATION_KIND_GROUND {
        let speed_y = if !VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
        ...
        let air_accel_y = if !VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
        ...
        let air_max_speed_y = if !VarModule::is_flag(fighter.module_accessor, vars::ken::status::flag::QUICK_STEP_INHERITED) {
