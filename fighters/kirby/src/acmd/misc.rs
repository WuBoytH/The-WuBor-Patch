...
unsafe extern "C" fn game_appeals(agent: &mut L2CAgentBase) {
    ...
    && !VarModule::is_flag(agent.module_accessor, vars::appeal::flag::LOOP) {
    ...
    if macros::is_excute(agent) {
        ...
        let sound_level;
        let sound;
        let loops = VarModule::get_int(agent.module_accessor, vars::kirby::status::int::APPEAL_S_LOOP_COUNT);
...
unsafe extern "C" fn effect_appealsloop(agent: &mut L2CAgentBase) {
    loop {
        if macros::is_excute(agent) {
            VarModule::inc_int(agent.module_accessor, vars::kirby::status::int::APPEAL_S_LOOP_COUNT);
        }
