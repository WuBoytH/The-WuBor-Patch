...
extern "C" {
    #[link_name = "special_n_bird_handler"]
    fn special_n_bird_handler(fighter: &mut L2CFighterCommon, status: L2CValue);
}
        ...
        if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_PUNCH_TURN) {
            if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_CAPTAIN_STATUS_WORK_ID_FLAG_FALCON_PUNCH_CHECK_END) {
                ...
                if lr * stick_x <= turn_stick_x {
                    fighter.change_status(FIGHTER_KIRBY_STATUS_KIND_CAPTAIN_SPECIAL_N_TURN.into(), true.into());
                    return 0.into();
                }
...
pub fn install(agent: &mut smashline::Agent) {
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_CAPTAIN_SPECIAL_N, special_n_main);
    agent.status(Exec, *FIGHTER_KIRBY_STATUS_KIND_CAPTAIN_SPECIAL_N, special_n_exec);

    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_CAPTAIN_SPECIAL_N_TURN, special_n_turn_main);
    agent.status(Exec, *FIGHTER_KIRBY_STATUS_KIND_CAPTAIN_SPECIAL_N_TURN, special_n_exec);
}