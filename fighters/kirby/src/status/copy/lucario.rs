...
unsafe extern "C" fn kirby_lucario_special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(
        kirby_lucario_special_n_main_loop as *const () as _,
    ))
}
...
unsafe extern "C" fn kirby_lucario_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !is_end {
        ...
        fighter.change_status(
            FIGHTER_KIRBY_STATUS_KIND_LUCARIO_SPECIAL_N_HOLD.into(),
            false.into(),
        );
    }
    0.into()
...
unsafe extern "C" fn kirby_lucario_special_n_hold_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    ControlModule::set_add_jump_mini_button_life(fighter.module_accessor, 8);
    fighter.sub_shift_status_main(L2CValue::Ptr(
        kirby_lucario_special_n_hold_main_loop as *const () as _,
    ))
}

unsafe extern "C" fn kirby_lucario_special_n_hold_main_loop(
    ...
    let max_charge_frame = WorkModule::get_param_float(
        fighter.module_accessor,
        hash40("param_special_n"),
        hash40("max_charge_frame"),
    );
    let charge = fighter.global_table[STATUS_FRAME].get_f32();
    if charge >= max_charge_frame + 4.0 {
        fighter.change_status(
            FIGHTER_KIRBY_STATUS_KIND_LUCARIO_SPECIAL_N_SHOOT.into(),
            false.into(),
        );
    }
    0.into()
