...
unsafe extern "C" fn special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::wolf::status::flag::SPECIAL_N_CHECK_ANGLE) {
        ...
        && cancel {
            fighter.change_status(vars::kirby::status::WOLF_SPECIAL_N_CANCEL.into(), true.into());
            return 0.into();
        }

        let stick_y = ControlModule::get_stick_y(fighter.module_accessor);
        let (g, a) = if stick_y >= 0.5 {
            (hash40("wolf_special_n_hi"), hash40("wolf_special_air_n_hi"))
        }
        else if stick_y <= -0.5 {
            (hash40("wolf_special_n_lw"), hash40("wolf_special_air_n_lw"))
        }
        else {
...
unsafe extern "C" fn special_n_mot_helper(fighter: &mut L2CFighterCommon) {
    ...
    if [hash40("special_n"), hash40("special_air_n")].contains(&motion) {
...
unsafe extern "C" fn special_n_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[STATUS_KIND].get_i32() != vars::kirby::status::WOLF_SPECIAL_N_CANCEL {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_KIRBY_STATUS_KIND_WOLF_SPECIAL_N, special_n_main);
    agent.status(End, *FIGHTER_KIRBY_STATUS_KIND_WOLF_SPECIAL_N, special_n_end);
}