...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_N, koopa_special_n_main);
    agent.status(Exec, *FIGHTER_STATUS_KIND_SPECIAL_N, koopa_special_n_exec);
    agent.status(ExecStop, *FIGHTER_STATUS_KIND_SPECIAL_N, koopa_special_n_exec_stop);
}