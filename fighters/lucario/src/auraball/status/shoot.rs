...
unsafe extern "C" fn lucario_auraball_shoot_pre(weapon: &mut L2CWeaponCommon) -> L2CValue {
    if VarModule::is_flag(weapon.module_accessor, vars::lucario_auraball::instance::flag::SPIRIT_BOMB) {
        ...
        && VarModule::is_flag((*owner_object).module_accessor, vars::lucario::status::flag::SPECIAL_N_START_FROM_GROUND);
        ...
        || lucario {
            ...
            VarModule::on_flag(weapon.module_accessor, vars::lucario_auraball::status::flag::FROM_AIR);
...
unsafe extern "C" fn lucario_auraball_shoot_main_fastshift(weapon: &mut L2CWeaponCommon) -> L2CValue {
    if VarModule::is_flag(weapon.module_accessor, vars::lucario_auraball::instance::flag::SPIRIT_BOMB) {
        ...
        && !AttackModule::is_infliction(weapon.module_accessor, *COLLISION_KIND_MASK_REFLECTOR) {
            if !VarModule::is_flag(weapon.module_accessor, vars::lucario_auraball::status::flag::EXPLOSION) {
                WorkModule::set_int(weapon.module_accessor, 30, *WEAPON_INSTANCE_WORK_ID_INT_LIFE);
                VarModule::on_flag(weapon.module_accessor, vars::lucario_auraball::status::flag::EXPLOSION);
            }
        }
    }
    let mut x_val = 0.0;
