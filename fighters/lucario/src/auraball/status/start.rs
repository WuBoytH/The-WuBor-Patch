...
unsafe extern "C" fn lucario_auraball_start_end(weapon: &mut L2CWeaponCommon) -> L2CValue {
    if !VarModule::is_flag(weapon.module_accessor, vars::weapon::instance::flag::FROM_POCKET) {
        ...
        && sv_battle_object::kind(owner_id) == *FIGHTER_KIND_LUCARIO {
            ...
            if VarModule::is_flag((*owner_object).module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB) {
                VarModule::on_flag(weapon.module_accessor, vars::lucario_auraball::instance::flag::SPIRIT_BOMB);
...
pub fn install(agent: &mut Agent) {
    agent.status(End, *WEAPON_LUCARIO_AURABALL_STATUS_KIND_START, lucario_auraball_start_end);
}