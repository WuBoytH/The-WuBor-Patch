...
pub unsafe extern "C" fn lucario_drain_aura(fighter: &mut L2CAgentBase, drain_all: bool) -> bool {
    let aura_charge = VarModule::get_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL);
    if aura_charge > 0 {
        if drain_all {
            VarModule::set_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL, 0);
            VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::AURA_ENHANCED_BY, aura_charge);
        }
        else {
            VarModule::dec_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL);
            VarModule::inc_int(fighter.module_accessor, vars::lucario::status::int::AURA_ENHANCED_BY);
        }
        true
    }
    else {
...
pub unsafe extern "C" fn lucario_gain_aura(fighter: &mut L2CAgentBase) -> bool {
    ...
    if VarModule::get_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL) < charge_max {
        VarModule::inc_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL);
...
pub unsafe extern "C" fn lucario_special_lw_eff_remover(fighter: &mut L2CAgentBase) {
    for x in vars::lucario::status::int::SPECIAL_LW_EFF1..=vars::lucario::status::int::SPECIAL_LW_EFF3 {
