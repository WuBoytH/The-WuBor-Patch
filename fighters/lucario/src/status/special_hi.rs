...
unsafe extern "C" fn lucario_special_hi_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    // let enhanced_by = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::AURA_ENHANCED_BY);
    // let cancel = if fighter.global_table[PREV_STATUS_KIND].get_i32() == *FIGHTER_LUCARIO_STATUS_KIND_SPECIAL_N_SHOOT {
    //     VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_ENABLE_SUPERDASH)
    // }
    // else { false };
    ...
    // VarModule::set_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_HI_CANCELLED_INTO, cancel);
    // if cancel {
    //     VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::AURA_ENHANCED_BY, enhanced_by);
    // }
    0.into()
}

unsafe extern "C" fn lucario_special_hi_init(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn lucario_special_hi_rush_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    StatusModule::init_settings(
        fighter.module_accessor,
        SituationKind(*SITUATION_KIND_NONE),
        *FIGHTER_KINETIC_TYPE_UNIQ,
...
unsafe extern "C" fn lucario_special_hi_rush_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !VarModule::is_flag(fighter.module_accessor, vars::lucario::instance::flag::EXTREME_SPEED_FORCE_NO_AURA)
...
unsafe extern "C" fn lucario_special_hi_rush_end_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if sit != *SITUATION_KIND_GROUND {
        ...
        if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
            let status = if !VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_HI_SUPER_DASH_CANCEL) {
...
unsafe extern "C" fn lucario_special_hi_rush_end_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_HI_SUPER_DASH_CANCEL) {
        if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_AIR {
            VarModule::on_flag(fighter.module_accessor, vars::lucario::instance::flag::FORCE_LANDING_FALL_SPECIAL);
        }
        lucario_special_hi_end(fighter, fighter.global_table[STATUS_KIND].clone());
    }
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_AIR {
        if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_HI_SUPER_DASH_CANCEL)
        && !VarModule::is_flag(fighter.module_accessor, vars::lucario::instance::flag::EXTREME_SPEED_FORCE_NO_AURA) {
            FighterSpecializer_Lucario::set_mach_validity(fighter.module_accessor, true);
            VarModule::on_flag(fighter.module_accessor, vars::lucario::instance::flag::EXTREME_SPEED_FORCE_NO_AURA);
        }
    }
    0.into()
}
