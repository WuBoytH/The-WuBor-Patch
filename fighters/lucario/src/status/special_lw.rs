...
unsafe extern "C" fn lucario_special_lw_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_STEP, vars::lucario::SPECIAL_LW_STEP_START);
...
unsafe extern "C" fn lucario_special_lw_substatus(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    let step = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_STEP);
    if param_1.get_bool() {
        if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_ENABLE_CANCEL) {
            ...
            if pad_flag & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 {
                ...
                VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_ATTACK);
            ...
            || fighter.sub_check_button_frick().get_bool() {
                ...
                VarModule::off_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_ENABLE_CANCEL);
                VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CANCEL);
                VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CHARGE_END);
                if fighter.sub_check_button_jump().get_bool()
                || fighter.sub_check_button_frick().get_bool() {
                    VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CANCEL_FORCE_JUMP);
                }
            }
        }
        if step == vars::lucario::SPECIAL_LW_STEP_CHARGE {
            lucario_special_lw_effect_helper(fighter);
            let charges_gained = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_CHARGES_GAINED);
            ...
            if VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_CHARGE_TIME) >= charge_max_total as i32 {
                VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_CHARGE_TIME, 0);
...
unsafe extern "C" fn lucario_special_lw_effect_helper(fighter: &mut L2CFighterCommon) {
    let level = VarModule::get_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL);
    if level > 2 {
        let eff = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF3) as u32;
        if !EffectModule::is_exist_effect(fighter.module_accessor, eff) {
            ...
            VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF3, eff as i32);
        }
    }
    if level > 1 {
        let eff = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF2) as u32;
        if !EffectModule::is_exist_effect(fighter.module_accessor, eff) {
            ...
            let eff = EffectModule::get_last_handle(fighter.module_accessor) as u32;
            VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF2, eff as i32);
        }
    }
    if level > 0 {
        let eff = VarModule::get_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF1) as u32;
        if !EffectModule::is_exist_effect(fighter.module_accessor, eff) {
            let lr = PostureModule::lr(fighter.module_accessor);
            EffectModule::req_follow(
                fighter.module_accessor,
            ...
            let eff = EffectModule::get_last_handle(fighter.module_accessor) as u32;
            VarModule::set_int(fighter.module_accessor, vars::lucario::status::int::SPECIAL_LW_EFF1, eff as i32);
        }
    }
}
...
unsafe extern "C" fn lucario_special_lw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if CancelModule::is_enable_cancel(fighter.module_accessor) {
        if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CANCEL_FORCE_JUMP) {
            ...
            VarModule::off_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CANCEL_FORCE_JUMP);
        }
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CHARGE_END) {
        let mot_g;
        let mot_a;
        if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_ATTACK) {
            mot_g = hash40("special_lw_attack");
            mot_a = hash40("special_air_lw_attack");
        }
        else if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CANCEL) {
        ...
        VarModule::off_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_LW_CHARGE_END);
    }
    if MotionModule::is_end(fighter.module_accessor) {
        if step == vars::lucario::SPECIAL_LW_STEP_END {
            let status = if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(End, *FIGHTER_STATUS_KIND_SPECIAL_LW, lucario_special_lw_end);
}