...
unsafe extern "C" fn lucario_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !is_end {
        ...
        && VarModule::get_int(fighter.module_accessor, vars::lucario::instance::int::AURA_LEVEL) > 2 {
            lucario_drain_aura(fighter, true);
            VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB);
...
unsafe extern "C" fn lucario_special_n_hold_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let ground_mot;
    let air_mot;
    if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB) {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_START_FROM_GROUND);
    }
    ControlModule::set_add_jump_mini_button_life(fighter.module_accessor, 8);
...
unsafe extern "C" fn lucario_special_n_shoot_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB_ENABLE_FALL) {
        VarModule::off_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB_ENABLE_FALL);
        if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_AIR {
            KineticModule::change_kinetic(fighter.module_accessor, *FIGHTER_KINETIC_TYPE_FALL);
            VarModule::on_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB_FALLING);
        }
    }
    ...
    if !MotionModule::is_end(fighter.module_accessor) {
        if !StatusModule::is_changing(fighter.module_accessor)
        && StatusModule::is_situation_changed(fighter.module_accessor) {
            if VarModule::is_flag(fighter.module_accessor, vars::lucario::status::flag::SPECIAL_N_SPIRIT_BOMB_FALLING)
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Main, *FIGHTER_LUCARIO_STATUS_KIND_SPECIAL_N_SHOOT, lucario_special_n_shoot_main);
}