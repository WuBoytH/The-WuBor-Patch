...
unsafe extern "C" fn lucina_attack_dash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !VarModule::is_flag(fighter.module_accessor, vars::yu::status::flag::ATTACK_DASH_BIG_GAMBLE)
    && fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI != 0
    && spent_meter(fighter.module_accessor, false) {
        VarModule::on_flag(fighter.module_accessor, vars::yu::status::flag::ATTACK_DASH_BIG_GAMBLE);
    }
    if VarModule::is_flag(fighter.module_accessor, vars::yu::status::flag::ATTACK_DASH_BIG_GAMBLE_TRANSITION)
    && VarModule::is_flag(fighter.module_accessor, vars::yu::status::flag::ATTACK_DASH_BIG_GAMBLE) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_ATTACK_DASH, lucina_attack_dash_main);
}