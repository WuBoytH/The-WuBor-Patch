...
unsafe extern "C" fn lucina_speciallw_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    VarModule::off_flag(fighter.module_accessor, vars::yu::status::flag::SPECIAL_LW_DECIDE_ROMAN_DIREC);
    VarModule::off_flag(fighter.module_accessor, vars::yu::status::flag::SPECIAL_LW_ROMAN_MOVE);
...
unsafe extern "C" fn lucina_speciallw_substatus(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::yu::status::flag::SPECIAL_LW_DECIDE_ROMAN_DIREC) {
        ...
        VarModule::set_float(fighter.module_accessor, vars::yu::status::float::SPECIAL_LW_ROMAN_MOVE, movement);
        VarModule::off_flag(fighter.module_accessor, vars::yu::status::flag::SPECIAL_LW_DECIDE_ROMAN_DIREC);
    }
    0.into()
}
...
unsafe extern "C" fn lucina_speciallw_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::yu::status::flag::SPECIAL_LW_ROMAN_MOVE) {
        let move_x = VarModule::get_float(fighter.module_accessor, vars::yu::status::float::SPECIAL_LW_ROMAN_MOVE);
        PostureModule::add_pos_2d(fighter.module_accessor, &Vector2f{
            x: move_x,
            y: 0.0
        });
        VarModule::set_float(fighter.module_accessor, vars::yu::status::float::SPECIAL_LW_ROMAN_MOVE, move_x * 0.9);
    }
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_LW, lucina_speciallw_main);
}