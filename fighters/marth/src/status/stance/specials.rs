...
unsafe extern "C" fn marth_speciallw_hit_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::on_flag(fighter.module_accessor, vars::marth::instance::flag::PARRY_XLU);
...
unsafe extern "C" fn marth_speciallw_hit_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if frame > 10.0
    && VarModule::is_flag(fighter.module_accessor, vars::marth::instance::flag::PARRY_XLU) {
...
unsafe extern "C" fn marth_speciallw_hit_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::marth::instance::flag::PARRY_XLU)
    && ![
        *FIGHTER_STATUS_KIND_SPECIAL_LW,
        *FIGHTER_STATUS_KIND_SPECIAL_HI,
        vars::marth::status::STANCE_DASH_F,
        vars::marth::status::STANCE_DASH_B,
        vars::marth::status::STANCE_ATTACK,
        vars::marth::status::STANCE_ATTACK_LW3,
        vars::marth::status::STANCE_ATTACK_HI3,
        vars::marth::status::STANCE_ATTACK_B3,
        vars::marth::status::STANCE_ATTACK_F3,
        vars::marth::status::STANCE_SPECIAL_S
...
unsafe extern "C" fn marth_speciallw_specials_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::off_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE);
    VarModule::on_flag(fighter.module_accessor, vars::marth::status::flag::DISABLE_STANCE_CHANGE);
...
unsafe extern "C" fn marth_speciallw_specials_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::marth::status::flag::SPECIAL_S_ENABLE_SPECIALS) {
        let cat1 = fighter.global_table[CMD_CAT1].get_i32();
        if cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI != 0 {
            VarModule::on_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE);
            fighter.change_status(FIGHTER_STATUS_KIND_SPECIAL_HI.into(), true.into());
            return 1.into();
        }
        if cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_S != 0 {
            VarModule::on_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE);
            fighter.change_status(vars::marth::status::STANCE_SPECIAL_S2_START.into(), true.into());
            return 1.into();
        }
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::marth::status::flag::SPECIAL_S_DASH) {
        fighter.change_status(vars::marth::status::STANCE_SPECIAL_S_DASH.into(), false.into());
    }
    0.into()
}
...
unsafe extern "C" fn marth_speciallw_specials_dash_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    || stick_y >= stick_y_threshold {
        ...
        VarModule::set_float(fighter.module_accessor, vars::marth::status::float::SPECIAL_S_ANGLE, angle);
...
unsafe extern "C" fn marth_speciallw_specials_dash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.module_accessor, vars::marth::status::flag::SPECIAL_S_END) {
        fighter.change_status(vars::marth::status::STANCE_SPECIAL_S_END.into(), false.into());
    }
    0.into()
}
...
unsafe extern "C" fn marth_speciallw_specials2_start_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::off_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE);
    VarModule::on_flag(fighter.module_accessor, vars::marth::status::flag::DISABLE_STANCE_CHANGE);
    VarModule::set_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_LOOP_COUNT, 1);
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        VarModule::set_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_START_SITUATION, *SITUATION_KIND_AIR);
        GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        MotionModule::change_motion(
            fighter.module_accessor,
        ...
        VarModule::set_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_START_SITUATION, *SITUATION_KIND_GROUND);
        GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND_CLIFF_STOP));
        MotionModule::change_motion(
            fighter.module_accessor,
...
unsafe extern "C" fn marth_speciallw_specials2_start_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if MotionModule::is_end(fighter.module_accessor) {
        fighter.change_status(vars::marth::status::STANCE_SPECIAL_S2_LOOP.into(), false.into());
    }
    0.into()
}
...
unsafe extern "C" fn marth_speciallw_specials2_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    let started_ground = VarModule::get_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_START_SITUATION) == *SITUATION_KIND_GROUND;
    let loop_count = VarModule::get_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_LOOP_COUNT);
    if StatusModule::is_situation_changed(fighter.module_accessor) {
        if started_ground {
    ...
    && fighter.global_table[PAD_FLAG].get_i32() & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 {
        VarModule::on_flag(fighter.module_accessor, vars::marth::status::flag::SPECIAL_S2_FINAL_BLOW);
    }
    if MotionModule::is_end(fighter.module_accessor) {
        if VarModule::is_flag(fighter.module_accessor, vars::marth::status::flag::SPECIAL_S2_FINAL_BLOW) {
            fighter.change_status(vars::marth::status::STANCE_SPECIAL_S2_END2.into(), true.into());
            return 1.into();
            // placeholder
        }
        if started_ground {
            ...
            && loop_count < vl::param_stance::special_s2_ground_loop_max {
                ...
                VarModule::inc_int(fighter.module_accessor, vars::marth::status::int::SPECIAL_S2_LOOP_COUNT);
            }
            else {
                fighter.change_status(vars::marth::status::STANCE_SPECIAL_S2_END.into(), false.into());
            }
        }
        else {
            if ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL)
...
pub fn install(agent: &mut Agent) {
    ...
    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S, marth_speciallw_specials_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S, marth_speciallw_specials_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S_DASH, marth_speciallw_specials_dash_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S_DASH, marth_speciallw_specials_dash_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S_DASH, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S_END, marth_speciallw_specials_end_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S_END, marth_speciallw_specials_end_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S_END, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S2_START, marth_speciallw_specials2_start_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S2_START, marth_speciallw_specials2_start_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S2_START, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S2_LOOP, marth_speciallw_specials2_loop_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S2_LOOP, marth_speciallw_specials2_loop_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S2_LOOP, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S2_END, marth_speciallw_specials2_end_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S2_END, marth_speciallw_specials2_end_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S2_END, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SPECIAL_S2_END2, marth_speciallw_specials2_end2_pre);
    agent.status(Main, vars::marth::status::STANCE_SPECIAL_S2_END2, marth_speciallw_specials2_end2_main);
    agent.status(End, vars::marth::status::STANCE_SPECIAL_S2_END2, marth_stance_common_end);
}