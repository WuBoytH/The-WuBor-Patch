...
unsafe extern "C" fn marth_speciallw_enter_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        ...
        if speed_y < 0.0
        && !VarModule::is_flag(fighter.module_accessor, vars::marth::instance::flag::AIR_STANCE) {
            sv_kinetic_energy!(
                set_speed,
                fighter,
        ...
        VarModule::on_flag(fighter.module_accessor, vars::marth::instance::flag::AIR_STANCE);
    }
    else {
...
unsafe extern "C" fn marth_speciallw_wait_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        if fighter.sub_check_command_squat().get_bool() {
            fighter.change_status(vars::marth::status::STANCE_SQUAT.into(), false.into());
            return 0.into();
        }
    }
    if !VarModule::is_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE) {
        fighter.change_status(vars::marth::status::STANCE_EXIT.into(), true.into());
        return 1.into();
    }
    if StatusModule::is_situation_changed(fighter.module_accessor) {
        if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
...
unsafe extern "C" fn marth_speciallw_squat_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        fighter.change_status(vars::marth::status::STANCE_WAIT.into(), true.into());
        return 0.into();
    }
    if !VarModule::is_flag(fighter.module_accessor, vars::marth::instance::flag::IS_STANCE) {
        fighter.change_status(vars::marth::status::STANCE_EXIT.into(), true.into());
...
unsafe extern "C" fn marth_speciallw_squat_wait_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
        if !fighter.sub_check_command_squat().get_bool() {
            fighter.change_status(vars::marth::status::STANCE_SQUAT_RV.into(), false.into());
            return 0.into();
        }
    }
    else {
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, vars::marth::status::STANCE_ENTER, marth_speciallw_enter_pre);
    agent.status(Main, vars::marth::status::STANCE_ENTER, marth_speciallw_enter_main);
    agent.status(End, vars::marth::status::STANCE_ENTER, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_WAIT, marth_speciallw_wait_pre);
    agent.status(Main, vars::marth::status::STANCE_WAIT, marth_speciallw_wait_main);
    agent.status(End, vars::marth::status::STANCE_WAIT, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SQUAT, marth_speciallw_squat_pre);
    agent.status(Main, vars::marth::status::STANCE_SQUAT, marth_speciallw_squat_main);
    agent.status(End, vars::marth::status::STANCE_SQUAT, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SQUAT_WAIT, marth_speciallw_squat_pre);
    agent.status(Main, vars::marth::status::STANCE_SQUAT_WAIT, marth_speciallw_squat_wait_main);
    agent.status(End, vars::marth::status::STANCE_SQUAT_WAIT, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_SQUAT_RV, marth_speciallw_squat_pre);
    agent.status(Main, vars::marth::status::STANCE_SQUAT_RV, marth_speciallw_squat_rv_main);
    agent.status(End, vars::marth::status::STANCE_SQUAT_RV, marth_stance_common_end);

    agent.status(Pre, vars::marth::status::STANCE_EXIT, marth_speciallw_exit_pre);
    agent.status(Main, vars::marth::status::STANCE_EXIT, marth_speciallw_exit_main);
    agent.status(End, vars::marth::status::STANCE_EXIT, marth_stance_common_end);
}