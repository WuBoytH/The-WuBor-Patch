...
unsafe extern "C" fn pikachu_dengekidama_regular_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    if VarModule::is_flag(weapon.module_accessor, vars::pikachu_dengekidama::status::flag::SPEED_UP) {
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *WEAPON_PIKACHU_DENGEKIDAMA_STATUS_KIND_REGULAR, pikachu_dengekidama_regular_main);
}