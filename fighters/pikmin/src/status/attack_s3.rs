...
unsafe extern "C" fn pikmin_attack_s3_handle_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    let step = VarModule::get_int(fighter.module_accessor, vars::pikmin::status::int::ATTACK_S3_STEP);
    if step != vars::pikmin::ATTACK_S3_STEP_END {
        ...
        if ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
            ...
            step = vars::pikmin::ATTACK_S3_STEP_LOOP;
            WorkModule::enable_transition_term(fighter.module_accessor, *FIGHTER_STATUS_TRANSITION_TERM_ID_CONT_GUARD_ON);
            VarModule::inc_int(fighter.module_accessor, vars::pikmin::instance::int::ATTACK_S3_LOOP_COUNT);
        }
        else {
            mot = hash40("attack_s3_end");
            step = vars::pikmin::ATTACK_S3_STEP_END;
        ...
        VarModule::set_int(fighter.module_accessor, vars::pikmin::status::int::ATTACK_S3_STEP, step);
        false.into()
    }
    else {
...
unsafe extern "C" fn pikmin_attack_s3_end(fighter: &mut L2CFighterCommon) -> L2CValue {
    if fighter.global_table[STATUS_KIND].get_i32() != *FIGHTER_STATUS_KIND_GUARD_ON {
        VarModule::set_int(fighter.module_accessor, vars::pikmin::instance::int::ATTACK_S3_LOOP_COUNT, 0);
    }
