...
unsafe extern "C" fn richter_attack_main(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn richter_attack_mtrans(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && VarModule::is_flag(fighter.module_accessor, vars::richter::status::flag::ATTACK_JUST_INPUT) {
        ...
        VarModule::off_flag(fighter.module_accessor, vars::richter::status::flag::ATTACK_JUST_INPUT);
    }
    0.into()
}

pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_ATTACK, richter_attack_main);
}