...
unsafe extern "C" fn richter_attack_air_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && !VarModule::is_flag(fighter.module_accessor, vars::richter::status::flag::ATTACK_AIR_LW_IGNORE_BOUNCE)
    && WorkModule::is_flag(fighter.module_accessor, *FIGHTER_SIMON_STATUS_ATTACK_FLAG_HIT) {
        if ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
            VarModule::on_flag(fighter.module_accessor, vars::richter::status::flag::ATTACK_AIR_LW_IGNORE_BOUNCE);
        }
        else {
