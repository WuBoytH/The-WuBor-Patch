...
unsafe extern "C" fn rockman_airshooter_regular_init(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let accel_x;
    let limit_speed_x;
    match VarModule::get_int(weapon.module_accessor, vars::rockman_airshooter::instance::int::NUM) {
        1 => {
            speed_x = 0.1;
            accel_x = 0.035;
            limit_speed_x = 0.5;
...
unsafe extern "C" fn rockman_airshooter_regular_exec(weapon: &mut L2CWeaponCommon) -> L2CValue {
    if VarModule::is_flag(weapon.module_accessor, vars::rockman_airshooter::status::flag::MOVE) {
        ...
        VarModule::off_flag(weapon.module_accessor, vars::rockman_airshooter::status::flag::MOVE);
    }
    0.into()
}
