...
unsafe extern "C" fn rockman_special_n_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
    if !VarModule::is_flag(fighter.module_accessor, vars::rockman::instance::flag::CHARGE_SHOT_PLAYED_FX) {
...
unsafe extern "C" fn rockman_special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::on_flag(fighter.module_accessor, vars::rockman::status::flag::CHARGE_SHOT_KEEP_CHARGE);
    let charge_frame = VarModule::get_int(fighter.module_accessor, vars::rockman::instance::int::CHARGE_SHOT_FRAME);
