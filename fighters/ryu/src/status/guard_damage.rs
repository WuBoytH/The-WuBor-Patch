...
unsafe extern "C" fn ryu_guard_damage_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    && VarModule::is_flag(fighter.module_accessor, vars::ryu::instance::flag::DENJIN_CHARGE)
...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_GUARD_DAMAGE, ryu_guard_damage_main);
}