...
unsafe extern "C" fn ryu_special_lw_step_b_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::ryu::status::flag::SPECIAL_LW_IMPACT_REMOVE_ARMOR) {
        ...
        VarModule::off_flag(fighter.module_accessor, vars::ryu::status::flag::SPECIAL_LW_IMPACT_ENABLED_ARMOR);
        VarModule::off_flag(fighter.module_accessor, vars::ryu::status::flag::SPECIAL_LW_IMPACT_REMOVE_ARMOR);
    }
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_RYU_STATUS_KIND_SPECIAL_LW_STEP_B, ryu_special_lw_step_b_pre);
    agent.status(Init, *FIGHTER_RYU_STATUS_KIND_SPECIAL_LW_STEP_B, ryu_special_lw_step_b_init);
    agent.status(Main, *FIGHTER_RYU_STATUS_KIND_SPECIAL_LW_STEP_B, ryu_special_lw_step_b_main);
}