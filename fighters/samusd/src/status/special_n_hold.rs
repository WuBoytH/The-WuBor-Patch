...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_SAMUS_STATUS_KIND_SPECIAL_N_H, samusd_special_n_hold_main);
    agent.status(Exit, *FIGHTER_SAMUS_STATUS_KIND_SPECIAL_N_H, samusd_special_n_hold_exit);
}