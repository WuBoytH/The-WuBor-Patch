...
unsafe extern "C" fn sonic_special_air_s_start_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if MotionModule::is_end(fighter.module_accessor) {
        fighter.change_status(vars::sonic::status::SPECIAL_AIR_S_HOLD.into(), false.into());
    }
    0.into()
}

pub fn install(agent: &mut Agent) {
    agent.status(Pre, vars::sonic::status::SPECIAL_AIR_S_START, sonic_special_air_s_start_pre);
    agent.status(Main, vars::sonic::status::SPECIAL_AIR_S_START, sonic_special_air_s_start_main);
}