...
unsafe extern "C" fn toonlink_attack_air_lw_bounce(fighter: &mut L2CFighterCommon) {
    if MotionModule::motion_kind(fighter.module_accessor) == hash40("attack_air_lw") {
        if AttackModule::is_infliction(fighter.module_accessor, *COLLISION_KIND_MASK_HIT) {
            VarModule::on_flag(fighter.module_accessor, vars::toonlink::status::flag::ATTACK_AIR_LW_BOUNCE);
        }
        if VarModule::is_flag(fighter.module_accessor, vars::toonlink::status::flag::ATTACK_AIR_LW_BOUNCE) {
            ...
            && MotionModule::frame(fighter.module_accessor) < 65.0 {
                ...
                VarModule::off_flag(fighter.module_accessor, vars::toonlink::status::flag::ATTACK_AIR_LW_BOUNCE);
