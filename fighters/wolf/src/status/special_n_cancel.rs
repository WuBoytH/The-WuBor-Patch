...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, vars::wolf::status::SPECIAL_N_CANCEL, special_n_cancel_pre);
    agent.status(Main, vars::wolf::status::SPECIAL_N_CANCEL, special_n_cancel_main);
    agent.status(End, vars::wolf::status::SPECIAL_N_CANCEL, special_n_cancel_end);
}