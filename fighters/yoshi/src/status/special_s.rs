...
unsafe extern "C" fn yoshi_special_s_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.module_accessor, vars::yoshi::status::flag::SPECIAL_S_HOP) {
        VarModule::off_flag(fighter.module_accessor, vars::yoshi::status::flag::SPECIAL_S_HOP);
        if fighter.global_table[SITUATION_KIND].get_i32() == *SITUATION_KIND_GROUND {
...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_STATUS_KIND_SPECIAL_S, yoshi_special_s_pre);
    agent.status(Init, *FIGHTER_STATUS_KIND_SPECIAL_S, yoshi_special_s_init);
    agent.status(Main, *FIGHTER_STATUS_KIND_SPECIAL_S, yoshi_special_s_main);
    agent.status(Exec, *FIGHTER_STATUS_KIND_SPECIAL_S, yoshi_special_s_exec);
    agent.status(End, *FIGHTER_STATUS_KIND_SPECIAL_S, yoshi_special_s_end);
}