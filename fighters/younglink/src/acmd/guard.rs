...
unsafe extern "C" fn effect_guardcancelattack(agent: &mut L2CAgentBase) {
    ...
    frame(agent.lua_state_agent, 4.0);
    if macros::is_excute(agent) {
        macros::LANDING_EFFECT(agent, Hash40::new("sys_dash_smoke"), Hash40::new("top"), 0, 0, 2, 0, 0, 0, 0.8, 0, 0, 0, 0, 0, 0, false);
    }
...
unsafe extern "C" fn sound_guardcancelattack(agent: &mut L2CAgentBase) {
    ...
    frame(agent.lua_state_agent, 4.0);
    if macros::is_excute(agent) {
        macros::PLAY_SE(agent, Hash40::new("se_younglink_attack100end"));
    }
}
