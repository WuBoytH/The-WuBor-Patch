...
unsafe extern "C" fn captain_specialsstart(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        ...
        VarModule::on_flag(agent.module_accessor, captain::status::flag::SPECIAL_S_ENABLE_MANUAL_ATTACK);
    }
    ...
    if macros::is_excute(agent) {
        ...
        VarModule::off_flag(agent.module_accessor, captain::status::flag::SPECIAL_S_ENABLE_MANUAL_ATTACK);
