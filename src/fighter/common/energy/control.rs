...
unsafe fn update(energy: &mut FighterKineticEnergyControl, boma: &mut BattleObjectModuleAccessor) {
    ...
    let accel_diff = match reset_type {
        ...
        ItemLift => {
            ...
            if !zero && accel_add_x * PostureModule::lr(boma) <= 0.0 {
                energy.speed_max.x = 0.0;
                energy.speed_brake.x = 0.0;
                stick.x = accel_add_x; // not sure if this is accurate but it's what I think I saw in the code
                zero = true;
...
unsafe fn initialize(energy: &mut FighterKineticEnergyControl, boma: &mut BattleObjectModuleAccessor) {
    ...
    match reset_type {
        ...
        FlyAdjust => {
            ...
            let stable = WorkModule::get_param_float(boma, hash40("air_speed_x_stable"), 0);
            let brake = WorkModule::get_param_float(boma, hash40("air_brake_x"), 0);
            let limit = WorkModule::get_param_float(boma, hash40("common"), hash40("air_speed_x_limit"));
            let accel_mul = WorkModule::get_param_float(boma, hash40("air_accel_x_mul"), 0) * fly_data.speed_x_mul;
            let accel_add = WorkModule::get_param_float(boma, hash40("air_accel_x_add"), 0);
...
unsafe fn setup(energy: &mut FighterKineticEnergyControl, reset_type: EnergyControllerResetType, initial_speed: &Vector3f, _unk: u64, boma: &mut BattleObjectModuleAccessor) {
    energy.clear_energy();
    ...
    match reset_type {
        FallAdjust | FallAdjustNoCap | StopCeil | WallJump => {
            ...
            && energy.unk[2] == 0 {
                // let object = MiscModule::get_battle_object_from_id(boma.battle_object_id);
                // let is_jump = VarModule::is_flag(object, jump::flag::JUMP_FROM_SQUAT);
                // let stable_speed = if is_jump {
                //     WorkModule::get_param_float(boma, smash::hash40("jump_speed_x_max"), 0)
                // } else {
                //     WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0)
                // };
                // let stable_speed = WorkModule::get_param_float(boma, smash::hash40("jump_speed_x_max"), 0);
        ...
        MoveGround => {
            let new_speed = KineticEnergy::adjust_speed_for_ground_normal(&energy.speed, boma);
