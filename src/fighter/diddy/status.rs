...
#[status_script(agent = "diddy", status = FIGHTER_STATUS_KIND_ATTACK_DASH, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_PRE)]
unsafe fn diddy_attackdash_pre(fighter: &mut L2CFighterCommon) -> L2CValue {
...
#[status_script(agent = "diddy", status = FIGHTER_STATUS_KIND_ATTACK_DASH, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
unsafe fn diddy_attackdash_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.sub_shift_status_main(L2CValue::Ptr(diddy_attackdash_main_loop as *const () as _))
}

unsafe extern "C" fn diddy_attackdash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
pub fn install() {
    install_status_scripts!(
        diddy_attackdash_pre,
        diddy_attackdash_main
    );
}
