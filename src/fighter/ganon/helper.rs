...
pub unsafe extern "C" fn deception_init(fighter: &mut L2CFighterCommon) {
    ...
    VarModule::set_vec2(fighter.module_accessor, ganon::status::float::TELEPORT_END_POS, Vector2f{x: tele_x, y: tele_y});
}
...
pub unsafe extern "C" fn deception_feint_handler(fighter: &mut L2CFighterCommon) {
    if VarModule::is_flag(fighter.module_accessor, ganon::status::flag::TELEPORT_FEINT) {
        ...
        if VarModule::is_flag(fighter.module_accessor, ganon::status::flag::TELEPORT_START_GROUND) {
        ...
        let tele_y = VarModule::get_vec2(fighter.module_accessor, ganon::status::float::TELEPORT_END_POS).y;
        if VarModule::is_flag(fighter.module_accessor, ganon::status::flag::TELEPORT_START_GROUND)
        && tele_y == 0.0 {
            GroundModule::set_correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
        }
    ...
    VarModule::set_int(fighter.module_accessor, ganon::status::int::TELEPORT_STEP, ganon::TELEPORT_STEP_END);
}
