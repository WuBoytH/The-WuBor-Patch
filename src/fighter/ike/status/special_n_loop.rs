...
#[status("ike", FIGHTER_IKE_STATUS_KIND_SPECIAL_N_LOOP)]
unsafe fn ike_special_n_loop_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    fighter.global_table[SUB_STATUS].assign(&L2CValue::Ptr(ike_special_n_loop_substatus as *const () as _));
    ike_special_n_loop_mot_helper(fighter);
    fighter.sub_shift_status_main(L2CValue::Ptr(ike_special_n_loop_main_loop as *const () as _))
}

unsafe extern "C" fn ike_special_n_loop_substatus(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if param_1.get_bool() {
...
unsafe extern "C" fn ike_special_n_loop_mot_helper(fighter: &mut L2CFighterCommon) {
    let charge_speed_mul = WorkModule::get_param_float(fighter.module_accessor, hash40("param_special_n"), hash40("charge_speed_mul"));
    let mot;
    let kinetic;
    let correct;
...
unsafe extern "C" fn ike_special_n_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if ControlModule::check_button_off(fighter.module_accessor, *CONTROL_PAD_BUTTON_SPECIAL) {
        ...
        // return 0.into();
        fighter.change_status(FIGHTER_IKE_STATUS_KIND_SPECIAL_N_END.into(), false.into());
        return 0.into();
    }
    if (special_n_charge_count_max as f32 * 30.0) <= charge_count {
        ...
        // fighter.change_status(FIGHTER_IKE_STATUS_KIND_SPECIAL_N_END_MAX.into(), false.into());
        fighter.change_status(FIGHTER_IKE_STATUS_KIND_SPECIAL_N_END.into(), false.into());
        return 0.into();
    }
...
pub fn install() {
    ike_special_n_loop_main::install();
}