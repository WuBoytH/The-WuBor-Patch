...
#[status("jack", FIGHTER_JACK_STATUS_KIND_SPECIAL_N_JUMP)]
pub unsafe fn jack_special_n_jump_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let additions = if WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_CUSTOMIZE_SPECIAL_N_NO) == 0 {
    ...
    if !StopModule::is_stop(fighter.module_accessor) {
        jack_special_n_jump_substatus(fighter, false.into());
    }
    fighter.global_table[SUB_STATUS].assign(&L2CValue::Ptr(jack_special_n_jump_substatus as *const () as _));
    fighter.sub_shift_status_main(L2CValue::Ptr(jack_special_n_jump_main_loop as *const () as _))
}

unsafe extern "C" fn jack_special_n_jump_substatus(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if !param_1.get_bool()
...
unsafe extern "C" fn jack_special_n_jump_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        if jack_special_n_jump_check_next(fighter, 1.into()).get_bool() {
            return 0.into();
        }
    }
            ...
            if escape_num < escape_num_max {
                if !WorkModule::is_flag(fighter.module_accessor, *FIGHTER_JACK_STATUS_SPECIAL_N_FLAG_UNABLE_SPECIAL_N) {
                    ...
                    if cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_S != 0 {
                        ...
                        fighter.change_status(FIGHTER_JACK_STATUS_KIND_SPECIAL_N_ESCAPE.into(), true.into());
                        return true.into();
                    }
                }
            }
            ...
            if WorkModule::is_flag(fighter.module_accessor, *FIGHTER_JACK_STATUS_SPECIAL_N_FLAG_CHECK_BARRAGE_BUTTON_ON) {
                if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
                    ...
                    || stick_dir == 3 {
                        ...
                        fighter.change_status(FIGHTER_JACK_STATUS_KIND_SPECIAL_N_BARRAGE_START.into(), true.into());
                        return true.into();
                    }
                }
            }
...
pub fn install() {
    jack_special_n_jump_main::install();
}