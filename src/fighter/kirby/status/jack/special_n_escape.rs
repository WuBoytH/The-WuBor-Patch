...
#[status_script(agent = "kirby", status = FIGHTER_KIRBY_STATUS_KIND_JACK_SPECIAL_N_ESCAPE, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
pub unsafe fn kirby_jack_special_n_escape_main(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe extern "C" fn kirby_jack_special_n_escape_substatus(fighter: &mut L2CFighterCommon, param_1: L2CValue) -> L2CValue {
    if param_1.get_bool() {
        ...
        kirby_jack_special_n_substatus_stick_check(fighter);
    }
    0.into()
}

unsafe extern "C" fn kirby_jack_special_n_escape_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        if kirby_jack_special_n_check_next(fighter, L2CValue::Ptr(kirby_jack_special_n_escape_next_status as *const () as _), 1.into()).get_bool() {
            return 0.into();
        }
    }
...
unsafe extern "C" fn kirby_jack_special_n_escape_next_status(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.change_status(FIGHTER_KIRBY_STATUS_KIND_JACK_SPECIAL_N.into(), true.into());
    0.into()
}
