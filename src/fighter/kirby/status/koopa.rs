...
unsafe fn kirby_koopa_special_n_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    kirby_koopa_special_n_handle_motion(fighter);
    fighter.sub_shift_status_main(L2CValue::Ptr(kirby_koopa_special_n_main_loop as *const () as _))
}

unsafe extern "C" fn kirby_koopa_special_n_mtrans_reset(fighter: &mut L2CFighterCommon) {
...
unsafe extern "C" fn kirby_koopa_special_n_handle_motion(fighter: &mut L2CFighterCommon) {
...
unsafe extern "C" fn kirby_koopa_special_n_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
...
unsafe fn kirby_koopa_special_n_exec(_fighter: &mut L2CFighterCommon) -> L2CValue {
    0.into()
}

#[status_script(agent = "kirby", status = FIGHTER_KIRBY_STATUS_KIND_KOOPA_SPECIAL_N, condition = LUA_SCRIPT_STATUS_FUNC_EXEC_STOP)]
unsafe fn kirby_koopa_special_n_exec_stop(_fighter: &mut L2CFighterCommon) -> L2CValue {
    0.into()
}

pub fn install() {
    install_status_scripts!(
        kirby_koopa_special_n_main,
        kirby_koopa_special_n_exec,
        kirby_koopa_special_n_exec_stop
    );
}