...
#[status("marth", FIGHTER_STATUS_KIND_SPECIAL_LW)]
...
#[status("marth", FIGHTER_MARTH_STATUS_KIND_SPECIAL_LW_HIT)]
unsafe fn marth_speciallw_hit_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::on_flag(fighter.battle_object, marth::instance::flag::PARRY_XLU);
...
unsafe extern "C" fn marth_speciallw_hit_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if frame > 10.0
    && VarModule::is_flag(fighter.battle_object, marth::instance::flag::PARRY_XLU) {
...
unsafe extern "C" fn marth_speciallw_specials_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.battle_object, marth::status::flag::SPECIAL_S_DASH) {
        let status = CustomStatusModule::get_agent_status_kind(fighter.battle_object, marth::status::STANCE_SPECIAL_S_DASH);
...
unsafe extern "C" fn marth_speciallw_specials_dash_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    || stick_y >= stick_y_threshold {
        ...
        VarModule::set_float(fighter.battle_object, marth::status::float::SPECIAL_S_ANGLE, angle);
...
unsafe extern "C" fn marth_speciallw_specials_dash_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if VarModule::is_flag(fighter.battle_object, marth::status::flag::SPECIAL_S_END) {
        let status = CustomStatusModule::get_agent_status_kind(fighter.battle_object, marth::status::STANCE_SPECIAL_S_END);
        fighter.change_status(status.into(), false.into());
    }
    0.into()
}
...
unsafe extern "C" fn marth_speciallw_specials2_start_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    VarModule::off_flag(fighter.battle_object, marth::instance::flag::IS_STANCE);
    VarModule::on_flag(fighter.battle_object, marth::status::flag::DISABLE_STANCE_CHANGE);
    VarModule::set_int(fighter.battle_object, marth::status::int::SPECIAL_S2_LOOP_COUNT, 1);
    if fighter.global_table[SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
        VarModule::set_int(fighter.battle_object, marth::status::int::SPECIAL_S2_START_SITUATION, *SITUATION_KIND_AIR);
        GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_AIR));
        MotionModule::change_motion(
            fighter.module_accessor,
        ...
        VarModule::set_int(fighter.battle_object, marth::status::int::SPECIAL_S2_START_SITUATION, *SITUATION_KIND_GROUND);
        GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND_CLIFF_STOP));
        MotionModule::change_motion(
            fighter.module_accessor,
...
unsafe extern "C" fn marth_speciallw_specials2_start_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    if MotionModule::is_end(fighter.module_accessor) {
        let status = CustomStatusModule::get_agent_status_kind(fighter.battle_object, marth::status::STANCE_SPECIAL_S2_LOOP);
        fighter.change_status(status.into(), false.into());
    }
    0.into()
}
...
unsafe extern "C" fn marth_speciallw_specials2_loop_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    let started_ground = VarModule::get_int(fighter.battle_object, marth::status::int::SPECIAL_S2_START_SITUATION) == *SITUATION_KIND_GROUND;
    let loop_count = VarModule::get_int(fighter.battle_object, marth::status::int::SPECIAL_S2_LOOP_COUNT);
    if StatusModule::is_situation_changed(fighter.module_accessor) {
        if started_ground {
    ...
    && fighter.global_table[PAD_FLAG].get_i32() & *FIGHTER_PAD_FLAG_ATTACK_TRIGGER != 0 {
        VarModule::on_flag(fighter.battle_object, marth::status::flag::SPECIAL_S2_FINAL_BLOW);
    }
    if MotionModule::is_end(fighter.module_accessor) {
        if VarModule::is_flag(fighter.battle_object, marth::status::flag::SPECIAL_S2_FINAL_BLOW) {
            let status = CustomStatusModule::get_agent_status_kind(fighter.battle_object, marth::status::STANCE_SPECIAL_S2_END2);
            fighter.change_status(status.into(), true.into());
            return 1.into();
            // placeholder
        }
        if started_ground {
            ...
            && loop_count < vl::param_stance::special_s2_ground_loop_max {
                ...
                VarModule::inc_int(fighter.battle_object, marth::status::int::SPECIAL_S2_LOOP_COUNT);
            }
            else {
                let status = CustomStatusModule::get_agent_status_kind(fighter.battle_object, marth::status::STANCE_SPECIAL_S2_END);
                fighter.change_status(status.into(), false.into());
            }
        }
        else {
...
pub fn install() {
    marth_speciallw_end::install();
    marth_speciallw_hit_main::install();
    marth_speciallw_hit_end::install();
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S_DASH,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S_END,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S2_START,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S2_LOOP,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S2_END,
        StatusInfo::new()
    ...
    CustomStatusManager::add_new_agent_status_script(
        Hash40::new("fighter_kind_marth"),
        marth::status::STANCE_SPECIAL_S2_END2,
        StatusInfo::new()
