...
#[acmd_script( agent = "ness", script = "game_catchattack", category = ACMD_GAME, low_priority )]
unsafe fn ness_catchattack(agent: &mut L2CAgentBase) {
...
unsafe fn ness_catchattack_eff(agent: &mut L2CAgentBase) {
    frame(agent.lua_state_agent, 4.0);
    if macros::is_excute(agent) {
...
#[acmd_script( agent = "ness", script = "sound_catchattack", category = ACMD_SOUND, low_priority )]
unsafe fn ness_catchattack_snd(agent: &mut L2CAgentBase) {
...
pub fn install() {
    install_acmd_scripts!(
        ness_catchattack, ness_catchattack_eff, ness_catchattack_snd
    );
}