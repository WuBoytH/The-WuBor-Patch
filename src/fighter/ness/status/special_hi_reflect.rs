...
unsafe extern "C" fn lucas_special_hi_reflect_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let special_hi_num = WorkModule::get_int(fighter.module_accessor, *FIGHTER_NESS_STATUS_SPECIAL_HI_WORK_INT_NUM);
    if special_hi_num != 0 && fighter.global_table[CMD_CAT1].get_i32() & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_HI != 0 {
        fighter.change_status(FIGHTER_NESS_STATUS_KIND_SPECIAL_HI_AGAIN.into(), true.into());
        return 1.into();
    }
    ...
    if MotionModule::is_end(fighter.module_accessor) {
        let status = WorkModule::get_int(fighter.module_accessor, *FIGHTER_NESS_STATUS_SPECIAL_HI_WORK_INT_NEXT_STATUS);
