...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_AIR, rockman_rockbuster_shoot_air_pre);
    agent.status(Main, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_AIR, rockman_rockbuster_shoot_air_main);
}