...
pub fn install(agent: &mut Agent) {
    agent.status(Pre, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WAIT, rockman_rockbuster_shoot_wait_pre);
    agent.status(Main, *FIGHTER_ROCKMAN_STATUS_KIND_ROCKBUSTER_SHOOT_WAIT, rockman_rockbuster_shoot_wait_main);
}