...
#[status_script(agent = "ryu", status = FIGHTER_STATUS_KIND_GUARD, condition = LUA_SCRIPT_STATUS_FUNC_STATUS_MAIN)]
unsafe fn ryu_guard_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.sub_status_guard_common();
    fighter.sub_shift_status_main(L2CValue::Ptr(ryu_guard_main_loop as *const () as _))
}

unsafe fn ryu_guard_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
