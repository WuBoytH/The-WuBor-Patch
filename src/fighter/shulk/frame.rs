...
unsafe extern "C" fn shulk_frame(fighter: &mut L2CFighterCommon) {
    ...
    if StatusModule::status_kind(fighter.module_accessor) == *FIGHTER_STATUS_KIND_REBIRTH {
        ...
        VarModule::set_float(fighter.module_accessor, shulk::instance::float::BURST_COOLDOWN, 0.0);
    }
    ...
    && !VarModule::is_flag(fighter.module_accessor, fighter::instance::flag::DISABLE_SPECIAL_LW) {
        ...
        VarModule::set_float(fighter.module_accessor, shulk::instance::float::BURST_COOLDOWN, 3600.0);
    }

    // Special Lw Check
    let mut burst_cooldown = VarModule::get_float(fighter.module_accessor, shulk::instance::float::BURST_COOLDOWN);
    if burst_cooldown > 0.0 {
        VarModule::sub_float(fighter.module_accessor, shulk::instance::float::BURST_COOLDOWN, 1.0);
        burst_cooldown = VarModule::get_float(fighter.module_accessor, shulk::instance::float::BURST_COOLDOWN);
        if burst_cooldown <= 0.0 {
