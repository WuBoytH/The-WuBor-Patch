...
#[acmd("sonic", "game_throwf")]
unsafe fn sonic_throwf(agent: &mut L2CAgentBase) {
...
#[acmd("sonic", "effect_throwf")]
unsafe fn sonic_throwf_eff(agent: &mut L2CAgentBase) {
...
#[acmd("sonic", "sound_throwf")]
unsafe fn sonic_throwf_snd(agent: &mut L2CAgentBase) {
...
#[acmd("sonic", "expression_throwf")]
unsafe fn sonic_throwf_exp(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
...
#[acmd("sonic", "game_throwlw")]
unsafe fn sonic_throwlw(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
...
pub fn install() {
    sonic_throwf::install();
    sonic_throwf_eff::install();
    sonic_throwf_snd::install();
    sonic_throwf_exp::install();
    sonic_throwlw::install();
}