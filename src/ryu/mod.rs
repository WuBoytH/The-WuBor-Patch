    ...
    unsafe {
        ...
        if entry_id(boma) < 8 {
            ...
            if StatusModule::status_kind(boma) == *FIGHTER_STATUS_KIND_REBIRTH || sv_information::is_ready_go() == false {
                ...
                SECRET_SENSATION[entry_id(boma)] = false;
                SEC_SEN_STATE[entry_id(boma)] = false;
            ...
            if EX_FLASH[entry_id(boma)] {
                if SEC_SEN_STATE[entry_id(boma)] {
                    if _TIME_COUNTER[entry_id(boma)] < 0 {
                        _TIME_COUNTER[entry_id(boma)] = 8;
                    }
                    if _TIME_COUNTER[entry_id(boma)] <= 4 {
                    ...
                    if _TIME_COUNTER[entry_id(boma)] < 0 {
                        _TIME_COUNTER[entry_id(boma)] = 12;
                    }
            ...
            if IS_FUNNY[entry_id(boma)] {
                ...
                if SECRET_SENSATION[entry_id(boma)] {
                    ...
                    if CAMERA[entry_id(boma)] == false {
                        ...
                        RYU_X[entry_id(boma)] = PostureModule::pos_x(boma);
                        RYU_Y[entry_id(boma)] = PostureModule::pos_y(boma);
                        if RYU_X[entry_id(boma)] == OPPONENT_X[entry_id(boma)] {
                            OPPONENT_DIRECTION[entry_id(boma)] = 12.0 * PostureModule::lr(boma);
                            SEC_SEN_DIREC[entry_id(boma)] = *FIGHTER_RYU_STATUS_KIND_SPECIAL_LW_STEP_F;
                        }
                        else if RYU_X[entry_id(boma)] < OPPONENT_X[entry_id(boma)] {
                            OPPONENT_DIRECTION[entry_id(boma)] = 12.0;
                            if PostureModule::lr(boma) == -1.0 {
                        ...
                        if OPPONENT_BOMA[entry_id(boma)] != 0 {
                            if (RYU_Y[entry_id(boma)] - OPPONENT_Y[entry_id(boma)]).abs() <= 12.0
                            && StatusModule::situation_kind(OPPONENT_BOMA[entry_id(boma)] as *mut BattleObjectModuleAccessor) == *SITUATION_KIND_GROUND {
                                VERT_EXTRA[entry_id(boma)] = 0.0;
                            }
                        }
                        else {
                            StatusModule::set_situation_kind(boma, SituationKind(*SITUATION_KIND_AIR), true);
                            WorkModule::on_flag(boma, *FIGHTER_STATUS_WORK_ID_FLAG_RESERVE_GRAVITY_STABLE_UNABLE);
                            VERT_EXTRA[entry_id(boma)] = 12.0;
                            RYU_Y[entry_id(boma)] += 2.0;
                            PostureModule::add_pos_2d(boma, &Vector2f{
                                x: 0.0,
                                y: 2.0
                            });
                        }
                        CAMERA[entry_id(boma)] = true;
                    }
                    if SEC_SEN_TIMER[entry_id(boma)] >= 0.0 {
                        if OPPONENT_BOMA[entry_id(boma)] != 0 {
                            if (RYU_Y[entry_id(boma)] - OPPONENT_Y[entry_id(boma)]).abs() <= 12.0
                            && StatusModule::situation_kind(OPPONENT_BOMA[entry_id(boma)] as *mut BattleObjectModuleAccessor) == *SITUATION_KIND_GROUND {
                                GroundModule::correct(boma, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
                        ...
                        if StatusModule::status_kind(boma) != SEC_SEN_DIREC[entry_id(boma)] {
                            KineticModule::change_kinetic(boma, *FIGHTER_KINETIC_TYPE_RESET);
                            StatusModule::change_status_request_from_script(boma, SEC_SEN_DIREC[entry_id(boma)], true);
                        }
                        if (RYU_Y[entry_id(boma)] - OPPONENT_Y[entry_id(boma)]).abs() > 12.0 {
                            StatusModule::set_situation_kind(boma, SituationKind(*SITUATION_KIND_AIR), true);
                        }
                        PostureModule::set_pos_2d(boma, &Vector2f{
                            x: (((OPPONENT_X[entry_id(boma)] + OPPONENT_DIRECTION[entry_id(boma)]) * SEC_SEN_TIMER[entry_id(boma)]) + RYU_X[entry_id(boma)] * (1.0 - SEC_SEN_TIMER[entry_id(boma)])),
                            y: (((OPPONENT_Y[entry_id(boma)] + VERT_EXTRA[entry_id(boma)]) * SEC_SEN_TIMER[entry_id(boma)]) + RYU_Y[entry_id(boma)] * (1.0 - SEC_SEN_TIMER[entry_id(boma)]))
                        });
                    }
                    SEC_SEN_TIMER[entry_id(boma)] += 0.08;
                    if SEC_SEN_TIMER[entry_id(boma)] > 1.0 {
                        SECRET_SENSATION[entry_id(boma)] = false;
                        CAMERA[entry_id(boma)] = false;
                        ...
                        SEC_SEN_TIMER[entry_id(boma)] = -0.6;
                        OPPONENT_BOMA[entry_id(boma)] = 0;
                    }
                }
                ...
                || MotionModule::motion_kind(boma) == hash40("appeal_hi_l") {
                    if MotionModule::frame(boma) == 4.0 {
                        ...
                        SEC_SEN_STATE[entry_id(boma)] = true;
                        EX_FLASH[entry_id(boma)] = true;
                        _TIME_COUNTER[entry_id(boma)] = -1;
                ...
                else if SECRET_SENSATION[entry_id(boma)] == false
                && SEC_SEN_STATE[entry_id(boma)] {
                    DamageModule::set_damage_lock(boma, false);
