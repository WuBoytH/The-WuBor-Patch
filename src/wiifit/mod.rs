...
unsafe fn wiifit_frame(fighter: &mut L2CFighterCommon) {
    ...
    if commonfuncs::get_player_number(boma) < 8 {
        ...
        if StatusModule::status_kind(boma) == *FIGHTER_STATUS_KIND_REBIRTH || smash::app::sv_information::is_ready_go() == false {
            DRAGON_INSTALL[commonfuncs::get_player_number(boma)] = false;
            DI_FLASH[commonfuncs::get_player_number(boma)] = false;
            CAN_DRAGON_INSTALL[commonfuncs::get_player_number(boma)] = true;
        }
        ...
        if IS_FUNNY[commonfuncs::get_player_number(boma)] {
            ...
            || MotionModule::motion_kind(boma) == smash::hash40("special_air_lw_success_l") {
                if MotionModule::frame(boma) == 1.0 {
                    DRAGON_INSTALL[commonfuncs::get_player_number(boma)] = true;
        ...
        if DRAGON_INSTALL[commonfuncs::get_player_number(boma)]
        && DI_FLASH[commonfuncs::get_player_number(boma)] == false
        ...
        if DI_FLASH[commonfuncs::get_player_number(boma)] {
